...
function prepare_partitions() {
	...
	mountopts[ext4]=',commit=15,errors=remount-ro' # EXT4 default: 5 (https://www.man7.org/linux/man-pages/man5/ext4.5.html)
	# mountopts[ext2] is empty
	# mountopts[fat] is empty
	# mountopts[f2fs] is empty
	mountopts[btrfs]=',commit=15' # BTRFS default: 30 (https://btrfs.readthedocs.io/en/latest/ch-mount-options.html)
	...
	if [[ -n $rootpart ]]; then
		...
		if [[ $ROOTFS_TYPE == btrfs ]]; then
			...
			mountopts[$ROOTFS_TYPE]=',commit=15'
	...
	if [[ -n $bootpart ]]; then
		...
		echo "UUID=$(blkid -s UUID -o value ${LOOP}p${bootpart}) /boot ${mkfs[$bootfs]} ro${mountopts[$bootfs]} 0 2" >> $SDCARD/etc/fstab
	fi
